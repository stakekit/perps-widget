...
export const selectedTokenBalanceAtom = Atom.family(
...
export type DepositFormBuilder = typeof depositFormBuilder;

const depositOnSubmit = Effect.gen(function* () {
...
export const createDepositForm = (
  ...
  FormReact.make(depositFormBuilder, {
    runtime: runtimeAtom,
    fields: { Amount: amountFieldComponent },
    onSubmit: (_, { decoded }) =>
      Effect.gen(function* () {
        const { client, wallet, selectedTokenBalance, selectedProvider } =
          yield* depositOnSubmit;
...
export const useSetDepositAmount = () => {
