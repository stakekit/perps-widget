...
export const depositActionAtom = Atom.writable<ActionDto | null, ActionDto>(
  () => null,
  (ctx, value) => ctx.setSelf(value),
);

export const signDepositActionAtom = makeSignTransactionsAtom(
  Atom.make(
    Effect.fn(function* (ctx) {
      const action = ctx.get(depositActionAtom);
      if (!action) {
